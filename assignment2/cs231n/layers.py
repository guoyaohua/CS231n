...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for i in range(out_H):
        for j in range(out_W):
            ...
            for k in range(F):
                out[:,k,i,j] = (x_pad_masked * w[k,:,:,:]).sum(axis = (1,2,3))
    #print((b)[None, :, None, None].shape)
    out = out + b.reshape((1,b.shape[0],1,1))
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(H_out):
        for j in range(W_out):
            ...
            boolean_mask = (x_masked == (max_x_masked)[:,:,None,None])
            dx[:,:,i*stride:i*stride+HH,j*stride:j*stride+WW] += boolean_mask * (dout[:,:,i,j])[:,:,None,None]
